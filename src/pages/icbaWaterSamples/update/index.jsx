...
export default function Index({ isOpen, close, itemId, setIsRefresh }) {
  ...
  const getFormData = () => {
    API.IcbaWaterSamples.getSingleUpdate({ id: itemId })
      .then((response) => {
        const { data } = response;
        ...
        form.setFieldsValue({
          ...data,
          app_user_id_hash: data?.app_user_id_hash,
        });
      })
      .catch((error) => {
  ...
  const onFinish = async () => {
    try {
      ...
      await API.IcbaWaterSamples.update(
        {
          ...values,
  ...
  const tabItems = [
    {
      label: t("IcbaWaterSamples.headerContent.title"),
      key: "form",
    ...
    },
    {
      ...
      children: (
        <DyamicTableTemplate
          ...
          TableIdHash="Dk5e3_1u7qQe6JoIMeRFSA=="
          title={t("modulePages.ICBAWaterExample")}
  ...
  return (
    <InsertDrawer
      title={t("IcbaWaterSamples.headerContent.editTitle")}
      close={closePage}
      isOpen={isOpen}
