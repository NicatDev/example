...
const CreateModal = ({
  ...
  const getInititalFormData = () => {
    const column = columnDefs.find(item => item.field === itemId);
    if (column) {
      form.setFieldsValue({ name: column?.headerName,editable:!column?.readonly,input_type:column?.input_type, description:column?.description??'' });
    }
  }
  // ===== Effects =====
  useEffect(() => {
  ...
  const onFinish = () => {
    ...
    API.Services.updateColumn(data)
      .then(() => {
        refreshTableData();
        form.resetFields();
        setOpen(false);
